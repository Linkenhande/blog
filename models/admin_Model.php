...
class admin_Model extends query_orm {
	...
	public function add_new_post($data){
		return $this->query_insert_orm('INSERT INTO blog_post (title, minDesc, description)
                                        VALUES ("'.$this->slashes($data['title']).'",
                                        "'.$this->slashes($data['minDesc']).'",
                                        "'.$this->slashes($data['description']).'")');
	}
}
