...
class main_Controller extends root_Controller {
    ...
    public function index_Action($check) {

    	if(empty($check))
    		$check = '1';

		$blog_Model = new blog_Model();
		$data['posts'] = $blog_Model->getPost($check);
		$data['pages'] = $blog_Model->getItemPost();
		$data['page'] = $check;
		
        $this->theme_view('main_View', 'home_View', $data);
    }
	
	public function descpost_Action($check) {
        
		$blog_Model = new blog_Model();
		$data['posts'] = $blog_Model->getPostOne($check);
        $data['coments'] = $blog_Model->getComment($check);

        $this->theme_view('main_View', 'descpost_View', $data);
    }

    public function addcoments_Action($check) {
        
        $blog_Model = new blog_Model();
        $data['posts'] = $blog_Model->addComent($check, $_POST);
                ...
                if($password!==$akcept_password){
                    ...
                    try {
                        ...
                        if(empty($user_unique)){
                            ...
                            $this->get_mail(0, 'Регистрация на сайте talkingfish.esy.es', 'Для подтверждения регистрации перейдите по ссылке: http://talkingfish.esy.es/main/regverification/'.$token, $post_valid['email']);
                ...
                if($password!==$akcept_password){
                    ...
                    try {
                        ...
                        if(empty($user_unique)){
                            ...
                            $this->get_mail(0, 'Регистрация на сайте talkingfish.esy.es', 'Для подтверждения регистриции перейдите по ссылке: http://talkingfish.esy.es/main/regverification/'.$token, $post_valid['email']);
                ...
                if(empty($error_login) && empty($error_password)){
                    ...
                    try {
                            ...
                            if($user[0]['chat_user_activ'] != 1){
                                ...
                                $data['error'] = 'Пользователь не активен. Проверьте почтовый ящик и подтвердите активацию.';
